...
cutover_agent_service() {
  ...
  install -m 600 "$staged_config" "$config_file" || return 1
  install -m 600 "$staged_service" "$service_file" || return 1
  rm -f "$timer_file" "$watch_service_file" || return 1
  systemctl --user daemon-reload || return 1
  systemctl --user enable --now "$(basename "$service_file")" || return 1
  local attempt
  for attempt in {1..30}; do
    if systemctl --user is-active --quiet "$(basename "$service_file")" &&
       "$node_path" "$agent_cli" status >/dev/null 2>&1; then
      return 0
    fi
    sleep 1
  done
...
rollback_agent_install() {
  ...
  for target in "$service_file" "$timer_file" "$watch_service_file" "$config_file" "$state_file" "$queue_file" "$dead_letter_file"; do
    ...
    if [[ -f "$saved" ]]; then install -m 600 "$saved" "$target"; else rm -f "$target"; fi
  done
