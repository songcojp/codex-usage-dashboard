...
The Agent reads `session_index.jsonl` from Codex data directories. Each valid record supplies:
...
Resolved paths are deduplicated so multiple configured sources can share one Codex data directory. The implementation does not read Codex's internal SQLite database. This avoids database-locking behavior and coupling to an internal schema. Names unavailable from `session_index.jsonl` remain unknown.
...
session_index.jsonl
        |
        v
Agent index scanner and acknowledged-name state
        |
        v
...
At startup, the Agent reads every discovered index file in full. It also adds each index directory to the existing filesystem-watch roots and repeats the scan after relevant filesystem activity. The existing six-hour reconciliation cycle is the fallback for missed notifications.

For each scan, the Agent:

1. parses newline-terminated JSONL records;
2. ignores unrelated or malformed records and defers an incomplete final line until a later scan;
3. trims titles and rejects empty or over-length values;
4. deduplicates by task ID, retaining the record with the latest valid `updated_at`;
5. compares the result with locally persisted acknowledged-name state;
6. uploads only new or changed revisions in batches of at most 1,000;
7. updates acknowledged-name state only after the server acknowledges the corresponding item.
...
- Malformed local index records do not block valid records or usage collection.
...
- index discovery across configured paths, `CODEX_HOME`, and the default data directory;
